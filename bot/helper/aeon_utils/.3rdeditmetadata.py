...
async def change_metadata(file, dirpath, key):
    ...
    # FFmpeg Command to modify metadata
    cmd = [
        "ffmpeg",
        "-y",
        "-i",
        ...
        "-metadata",
        "encoded_by=",  # Explicitly set Encoded by field to an empty string
        "-metadata",
        "Telegram_Channel=@FiLiMHOUSE",  # Add new custom metadata field
        temp_file_path
    ]
    ...
    # Replace original file with the temp file after metadata modification
